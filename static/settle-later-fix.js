// Merged settle-later functionality with fixes
// Global variables
...
function initSettleLater() {
  ...
  // 2. Modify the proceed-checkout-btn click handler (use cloneNode to remove duplicate listeners)
  ...
  if (originalProceedCheckoutBtn) {
    ...
    newProceedBtn.addEventListener("click", async function () {
      console.log(
        "Proceed checkout clicked with settle later handler (merged)"
      );
      ...
      // Enhanced validation: block checkout for positive balance unless settle later is checked
      if (balance > 0 && !settleLaterEnabled) {
        showNotification(
          `Cannot checkout with pending balance of ₹${balance}. Please clear dues or enable 'Settle Later'.`,
          "error"
        );
        checkoutInProgress = false;
        this.disabled = false;
      ...
      // Block negative balances (refund) entirely
      if (balance < 0) {
        showNotification(
      ...
      // Get refund method if there's a negative balance (not expected due to earlier block)
      const refundMethod =
        balance < 0
  ...
  console.log("Settle Later feature initialized (merged)");
}
  ...
  if (typeof setupCheckoutConfirmation === "function") {
    ...
    window.setupCheckoutConfirmation = function () {
      ...
      if (confirmCheckoutBtn) {
        // Remove existing listeners and replace with a safe one
        ...
        newBtn.addEventListener(
          ...
          function (event) {
            ...
            if (checkoutConfirmModal) {
              ...
              console.log(
                "Confirmation modal displayed with enhanced validation"
              );
            }
          },
          true
