...
def verify_token(token: str) -> dict:
    ...
    try:
        # check_revoked=True ensures admin force-logout takes effect
        ...
        # Catch-all so a transient Firebase blip never leaks a stack trace
        logger.warning(f"verify_token: unexpected error: {e}")
...
def load_current_user() -> Optional[dict]:
    ...
    if not user_id or not role:
        # Token is valid but missing our custom claims — likely a user
        # created outside the seed/admin flow. Refuse.
        return None
