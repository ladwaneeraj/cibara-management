(function () {
  ...
  window.addEventListener("load", function () {
    ...
    if (originalProceedCheckoutBtn) {
      // If another script (settle-later.js) already attached a handler, skip adding a second one
      if (
        originalProceedCheckoutBtn.dataset &&
        originalProceedCheckoutBtn.dataset.settleLaterHandler === "true"
      ) {
        console.log(
          "Settle-later handler already attached; skipping enhanced proceed handler to avoid duplicate requests."
        );
      } else {
        ...
        newProceedBtn.addEventListener("click", async function () {
          ...
          console.log(
            `Balance: ${balance}, Settle Later: ${settleLaterEnabled}`
          );
